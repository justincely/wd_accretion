...
C     change m(1) here
C      if (time.le.365.d3) then
C        m(1)=K2-(0.46)*K2*(3*(time/365.d3)**2-2*(time/365.d3)**3)
C      end if
...
c Save the current heliocentric coordinates and velocities
     ...
     %  mem,lmem)
      time = time + h0
C     change m(1) here
C      if (time.le.365.d4) then
C        m(1)=K2-(0.46)*K2*(3*(time/365.d4)**2-2*(time/365.d4)**3)
C      end if
...
c Local
      ...
      real*8 t1
      character*38 flost,fcol,flostorb
      character*6 tstring
c
c------------------------------------------------------------------------------
c
...
c
      if (opt(3).eq.1) then
        ...
        if (i.eq.0) then
          ...
          write (23,*) xh(1,j),xh(2,j),xh(3,j)
          write (23,*) vh(1,j),vh(2,j),vh(3,j)
        else
